...
city_select_element = document.getElementById("city_select_element");
var city_select_element_children = city_select_element.children; // リセットボタン(「市を入力するボタン」をクリック後、再度「都道府県ボタン」をクリックすると、初期状態に戻す。)
  ...
  city_select_start_button.addEventListener('mouseup', function () {
    if (prefectures_select_button.textContent == '都道府県を選択してください') {} else {
      ...
      if (!city_select_button_fake_delete) {
        ...
        city_select_start_button.after(city_select_button_fake); // 元のselectタグを生成

        if (city_select_tag_create == "necessary") {
          for (var i = 0; i < city_select_element_children.length; i++) {
            city_select_element.appendChild(city_select_element_children[i]);
          }
        } // 「市」selectタグのoption選択データを追加(子要素)


        var city_select_option_elements_array = document.querySelectorAll("#".concat(prefectures_select_button.textContent));
...
/******/ });
//# sourceMappingURL=application-2049d8668ec5bd3aa6d9.js.map