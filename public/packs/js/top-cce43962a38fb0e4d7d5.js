...
function initializeLiff(myLiffId) {
  liff.init({
    ...
    var idToken, body, request, _idToken, _body, _request;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            if (liff.isInClient()) {
              // LIFFブラウザ内で動作
              idToken = liff.getIDToken(); // lineのユーザーIDトークンを取得する

              body = "idToken=".concat(idToken);
              ...
              if (liff.isLoggedIn()) {
                ...
                _idToken = liff.getIDToken(); // lineのユーザーIDトークンを取得する

                _body = "idToken=".concat(_idToken);
                _request = new Request('/users', {
                  headers: {
                    'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8'
                  },
                  method: 'POST',
                ...
                fetch(_request).then(function (response) {// このブロックの中ではPromiseではなくて、通常の値として扱える
                  //if (response.status == 500) {
                  //  document.getElementById("liffAppContent").classList.add('hidden');
                  // document.getElementById("liffErrorMessage").classList.remove('hidden');
                  //}
...
/******/ });
//# sourceMappingURL=top-cce43962a38fb0e4d7d5.js.map