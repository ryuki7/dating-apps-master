...
var city_select_element_children = city_select_element_fake.children; // リセットボタン(「市を入力するボタン」をクリック後、再度「都道府県ボタン」をクリックすると、初期状態に戻す。)
...
if (prefectures_select_button) {
  prefectures_select_button.addEventListener('mousedown', function () {
    ...
    if (city_select_button_fake_delete) {
      ...
      city_select_button_fake_delete.remove(); // 元のselectタグ生成判別
...
if (prefectures_select_button && city_select_start_button) {
  city_select_start_button.addEventListener('mouseup', function () {
    if (prefectures_select_button.textContent == '都道府県を選択してください') {
      var _city_select_start_button = document.getElementById("city_select_start_button");

      var _error_message = document.createElement('p');

      _error_message.textContent = '「都道府県」を選択して下さい。';
      _error_message.id = 'targets_registration_city_input_error_message';

      _city_select_start_button.before(_error_message);
    } else {
...
function beforesubmit_select_check() {
  ...
  error_message.textContent = '「アプリ」・「容姿」・「目的」は選択必須です。';
  error_message.id = 'targets_registration_error_message';

  if (app_select_button.textContent == 'アプリを選択してください' && appearance_select_button.textContent == '容姿を選択してください' && purpose_select_button.textContent == '目的を選択してください') {
    targets_registration_submit_button.before(error_message);
  } else {
...
/******/ });
//# sourceMappingURL=application-37a96aea829018328638.js.map