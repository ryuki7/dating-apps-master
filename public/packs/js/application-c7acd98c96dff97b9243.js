...
(function ($) {
  $.fn.selModal = function () {
    ...
    SEL_PARENT.each(function (i) {
      ...
      if (selected_name === undefined) {
        ...
        if (i == 48) {
          button_html = '<button type="button" class="selModalButton" data-selmodalbtn="' + sel_name_attr + '" id="prefectures_select_button">' + selected_name + '</button>';
        } else if (select_this[0].id == "address_select_element") {
          button_html = '<button type="button" class="selModalButton hidden" data-selmodalbtn="' + sel_name_attr + '" id="address_select_button">' + selected_name + '</button>';
        } else {
      ...
      if (select_this[0].id == "prefectures_select_element") {} else {
  ...
  prefectures_select_button_create();
})(jQuery);

function prefectures_select_button_create() {
  var prefectures_select_button = document.getElementById("prefectures_select_button");

  if (prefectures_select_button) {
    sel_name_attr = "prefectures_select";
    var prefectures_select_element = document.getElementById("prefectures_select_element");
    prefectures_select_button.setAttribute("data-selmodalbtn", sel_name_attr); //ボタンクリック

    $('[data-selmodalbtn="' + sel_name_attr + '"]').click(function () {
      href = $('[data-selmodal="' + prefectures_select_element.data('selmodalbtn') + '"]');
      $(href).show();
      $('body').addClass('selModalBody');
      var par_height = $(href).find('.selModalInner').outerHeight();
      var c_head = $(href).find('.selModalHeader').outerHeight();
    ...
    $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList').on('touchstart', function () {
      if (prefectures_select_element.scrollTop() == 0) {
        prefectures_select_element.scrollTop(1);
      }

      var scrollHeight = prefectures_select_element.scrollHeight;
      var scrollPosition = prefectures_select_element.scrollTop() + prefectures_select_element.height();

      if (scrollHeight == scrollPosition) {
        prefectures_select_element.scrollTop(prefectures_select_element.scrollTop() - 1);
      }
    });
    $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList li').on('touchstart', function () {
      prefectures_select_element.addClass('totch');
    });
    $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList li').on('touchend', function () {
      prefectures_select_element.closest('ul').find('li').removeClass('totch');
    }); //ボタンクリック

    $('[data-selmodal="' + sel_name_attr + '"]').find('li').click(function () {
      prefectures_select_element.closest('ul').find('li').removeClass('selected');
      prefectures_select_element.addClass('selected');
      var hoge = prefectures_select_element.closest('.selModal').data('selmodal');
      var this_no = prefectures_select_element.closest('ul').find('li').index(prefectures_select_element);
      var before_no = $('select[name="' + hoge + '"]').prop("selectedIndex");
      $('select[name="' + hoge + '"]').prop("selectedIndex", this_no);
      $('[data-selmodalbtn="' + hoge + '"]').text(prefectures_select_element.text());
    ...
    $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalClose').click(function () {
      prefectures_select_element.parents(".selModal").hide();
      $('body').removeClass('selModalBody');
    });
  }
}

var address_select_element = document.getElementById("address_select_element");

if (address_select_element) {
  var address_select_button = address_select_element.nextElementSibling;
  address_select_button.classList.add('sw');
}
...
if (prefectures_select_button) {
  if (!prefectures_select_button.textContent == '都道府県を選択してください') {
    var _address_select_element = document.getElementById("address_select_element");

    var _address_select_button = _address_select_element.nextElementSibling;

    _address_select_button.classList.remove('sw');
  }
}
...
/******/ });
//# sourceMappingURL=application-c7acd98c96dff97b9243.js.map