 ...
 *  
 *--------------------------------------------------------------------------*/
var prefectures_select_button_create_OK = 0;
$(function () {
  //後者のセレクトをプラグインに適用
...
if (prefectures_select_button) {
  if (prefectures_select_button.textContent == '都道府県を選択してください') {} else {
    prefectures_select_button_create_OK = 1;

    $.fn.selModal = function () {
      var SEL_PARENT = $(this);
      SEL_PARENT.each(function (i) {
        var select_this = $('#city_select_element');
        var sel_name_attr = select_this.attr('name');
        var selected = select_this.prop('selectedIndex');
        var selected_name;
        var modal_html = '';
        var button_html = '';
        ...
        var aryChild = select_this.children();
        ...
        select_this.hide(); //ボタン生成

        if (select_this[0].id == "prefectures_select_element") {
          prefectures_select_button_create(sel_name_attr);
        } else if (select_this[0].id == "city_select_element" && prefectures_select_button_create_OK == 1) {
          city_select_button_create(sel_name_attr);
          prefectures_select_button_create_OK = 0;
        } else {
          select_this.after(button_html);
        } //モーダルウインドウ生成
        ...
        $('[data-selmodalbtn="' + sel_name_attr + '"]').click(function () {
          href = $('[data-selmodal="' + select_this.data('selmodalbtn') + '"]');
          $(href).show();
        ...
        $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList').on('touchstart', function () {
          if (select_this.scrollTop() == 0) {
            select_this.scrollTop(1);
          }

          var scrollHeight = select_this[0].scrollHeight;
          var scrollPosition = select_this.scrollTop() + select_this.height();

          if (scrollHeight == scrollPosition) {
            select_this.scrollTop(select_this.scrollTop() - 1);
          }
        });
        $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList li').on('touchstart', function () {
          select_this.addClass('totch');
        });
        $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalList li').on('touchend', function () {
          select_this.closest('ul').find('li').removeClass('totch');
        }); //ボタンクリック

        $('[data-selmodal="' + sel_name_attr + '"]').find('li').click(function () {
          select_this.closest('ul').find('li').removeClass('selected');
          select_this.addClass('selected');
          var hoge = select_this.closest('.selModal').data('selmodal');
          var this_no = select_this.closest('ul').find('li').index(select_this);
          var before_no = $('select[name="' + hoge + '"]').prop("selectedIndex");
          $('select[name="' + hoge + '"]').prop("selectedIndex", this_no);
          $('[data-selmodalbtn="' + hoge + '"]').text(select_this.text());
        ...
        $('[data-selmodal="' + sel_name_attr + '"]').find('.selModalClose').click(function () {
          select_this.parents(".selModal").hide();
...
/******/ });
//# sourceMappingURL=application-d0396f19cfb4c4e33be3.js.map